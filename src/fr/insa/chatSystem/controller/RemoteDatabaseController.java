...
public abstract class RemoteDatabaseController {
	...
	public static String getHistory(String username1, String username2) {
		try {
			ResultSet rs = statement
					.executeQuery("SELECT history FROM histories WHERE id1=(SELECT id FROM IDs WHERE username=\""
							+ username1 + "\") AND id2=(SELECT id FROM IDs WHERE username=\"" + username2 + "\");");
			while (rs.next()) {
	...
	public static String getIDNumber(String name_input) {
		return name_input;
